...
public abstract class AnySoftKeyboard extends InputMethodService implements
    ...
    private boolean noLocationAllowed = false;

    private ILocationPresenter mLocationPresenter = new ILocationPresenter() {
    ...
    public void onStartInputView(final EditorInfo attribute, final boolean restarting) {
        ...
        try {
            ...
            if (locationRequired != null) {
                ...
                showNotificationWithIntent(locationRequired);
                //and failing. So it will try to access the user's location again
                throw new RuntimeException("We do not have permission to read your location!");
            } else {
    ...
    public void onUpdateSelection(int oldSelStart, int oldSelEnd,
        ...
        if (BuildConfig.DEBUG) {
            Log.d(TAG, "onUpdateSelection: oss=%d, ose=%d, nss=%d, nse=%d, cs=%d, ce=%d",
    ...
    private void showNotificationWithIntent(Intent locationRequired) {
        final int requestId = PermissionsRequestCodes.LOCATION.getRequestCode();
        PendingIntent pendingIntent = PendingIntent.getActivity(mContext, requestId, locationRequired, 0);
        NotificationCompat.Builder builder = new NotificationCompat.Builder(mContext);
        builder.setTicker(mContext.getString(R.string.notification_location_ticker));
        builder.setSmallIcon(R.drawable.ic_notification_location_permission_required);
        builder.setContentIntent(pendingIntent);
        builder.setContentTitle(mContext.getString(R.string.notification_location_title));
        builder.setContentText(mContext.getString(R.string.notification_location_text));
    ...
    private void setSuggestions(List<SuggestionObject> suggestions,
    ...
    public void onFunctionKey(int primaryCode, Key key, int multiTapIndex, int[] nearByKeyCodes, boolean fromUI) {
        if (BuildConfig.DEBUG) {
            Log.d(TAG, "onFunctionKey %d", primaryCode);
        }
    ...
    /*package*/ void performUpdateSuggestions() {
        ...
        if (stringList.size() > 0) {
            if (correctionAvailable && !typedWordValid && stringList.size() > 1) {
                mWord.setPreferredWord(stringList.get(1).getSuggestedWord());
            } else {
